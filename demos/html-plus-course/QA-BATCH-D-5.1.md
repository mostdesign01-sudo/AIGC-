...
- 验收对象：`demos/html-plus-course/index.html`，分支 `cursor/html-plus-batch-d-a572`，HEAD `5c7510c`（PR #16，base = `cursor/html-plus-batch-c-59dc` @ `48e2852`，批次 C 修补后已通过）。
...
批次 D 四条成功标准（含 P1-3）里三条半满足：P02 / P09 / P18 / P19 的舞台节律 ✓，P09 一行说明 + 单开 ✓，390 全部 20 页无横向滚动、案例控件进页即在 dock 之上 ✓，演示输出不入库 + 重置 ✓，冻结句 / 口径 / 越批 ✓。

**一条不满足**：390 宽下 HUD 当前章标签 11px 做到了，但第 01 章「什么是 HTML」在 61px 的可用宽度里需要 64px，被 `text-overflow: ellipsis` 截成「什么是 HT…」（P03 / P04 / P05 三页都是这个状态，360 宽下更差：56px 装 64px）。成功标准写的是「当前章标签可读（≥ 11px）」，截断的章名不算可读。其余五章（开场 / 可以做什么 / 怎么选择 / 我们的案例 / 轮到你了）22–55px，都装得下。

这是一条 CSS 规则的修补，见第 2 节；QA 已在副本上预演，修完只需复量第 2.3 节两个数。**不开批次 E**，等 HUD 修补通过后再开。
...
**D-1 · 390 宽 HUD 当前章「什么是 HTML」被截成「什么是 HT…」**
...
3.9 **两个 commit**：`2fc1d53` + `5c7510c`（后者是 P07 手机端动作卡两列的补丁），与 §5.6「每批一个 commit」有出入；两者都在批次 D 范围内，不要求合并，D-1 修补按 §5.6 再追加一个即可。
...
- 基线仍是 `5c7510c`；只改第 2.1 节一行 CSS。
- 不碰 P02 / P09 / P18 / P19 已通过的版面、`revealAboveChrome / resetCourse / persistEdits`、七个 `data-frozen`、`editSel`、四处 `AI Coding`、20 个 `data-note`。
- commit 标题沿用 `HTML+ 批次D：版面节律与手机（修补 HUD 当前章标签）`，PR #16 描述追加 390 与 360 下六章 HUD 截图各一张（至少含「什么是 HTML」）。
- 修补通过后再开批次 E；批次 E 开工时把本文件 3.1（P09 省略号）、3.5（1280 P15）、3.7 / 3.8（README 快捷键）一并带上。
